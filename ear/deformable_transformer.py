...
from ear.ops.modules import MSDeformAttn


# Versatile Encoder-Decoder
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
                 num_encoder_layers=6, num_decoder_layers=6, dim_feedforward=1024, dropout=0.1,
                 activation="relu", return_intermediate_dec=False,
                 num_feature_levels=4, dec_n_points=4, enc_n_points=4, args=None):
        super().__init__()
        ...
        elif args.text_crossAttn is True and args.combined_encoder is False:
    ...
    def prepare_decoder_input_query(self, memory, query_embed):
        ...
        query_embed = query_embed.unsqueeze(0).expand(bs, -1, -1)  # [bs, num_rand_queries, d_model]
        tgt = tgt.unsqueeze(0).expand(bs, -1, -1)  # [bs, num_rand_queries, d_model]
...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
                 dropout=0.1, activation="relu",
                 n_levels=4, n_heads=8, n_points=4, args=None):
        super().__init__()
        ...
        if args.text_crossAttn:
            self.text_cross = MCA(dim=d_model, num_heads=n_heads)
    ...
    def forward(self, tgt, query_pos, reference_points, src, src_temporal_shapes, level_start_index,
                src_padding_mask=None, query_mask=None, text_feat=None):

        # tgt (1,10,512)
        # query_pos (1,10,512)
        # src (1,188,512)
        # reference_points (1,10,4,1)
        # text_feat (1,27*n,512)
        ...
        if self.args.text_crossAttn_loc == "before":

            if text_feat is not None:
                tgt2 = self.text_cross(tgt, text_feat, text_feat)
        ...
        if self.args.text_crossAttn_loc == "after":
            if text_feat is not None:
                tgt2 = self.text_cross(tgt, text_feat, text_feat)
                tgt = tgt + self.dropout_t(tgt2)
                tgt = self.norm1(tgt)
    ...
    def forward(self, tgt, reference_points, src, src_temporal_shapes, src_level_start_index, src_valid_ratios,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            output = layer(output, query_pos, reference_points_input, src, src_temporal_shapes, src_level_start_index,
                           src_padding_mask, query_padding_mask, text_feat)

...
class HungarianMatcher_cl(nn.Module):
    """This class computes an assignment between the targets and the predictions of the network
    ...
    def forward(self, outputs, targets, verbose=False, return_C=False):
        ...
        with torch.no_grad():
            ...
            if not isinstance(cost_cl, torch.Tensor):
                cost_cl = torch.tensor(cost_cl, device=cost_bbox.device, dtype=cost_bbox.dtype)
            if cost_cl.shape[0] != cost_bbox.shape[0]:
                repeat_times = (cost_bbox.shape[0] + cost_cl.shape[0] - 1) // cost_cl.shape[0]
                cost_cl = cost_cl.repeat(repeat_times, 1)[:cost_bbox.shape[0]]
            if cost_cl.shape[1] != cost_bbox.shape[1]:
                cost_cl = cost_cl[:, :cost_bbox.shape[1]]
            # ===========================================
            ...
            indices = [linear_sum_assignment(c[i]) for i, c in enumerate(C.split(sizes, -1))]
            m2o_rate = 4
            rl_indices = [linear_sum_assignment(torch.cat([c[i]] * m2o_rate, -1)) for i, c in
                          enumerate(C.split(sizes, -1))]
            rl_indices = [(torch.as_tensor(i, dtype=torch.int64), torch.as_tensor(j % sizes[ii], dtype=torch.int64)) for
                          ii, (i, j) in
                          enumerate(rl_indices)]
...
def build_matcher_cl(args):
    return HungarianMatcher_cl(cost_class=args.set_cost_class,
                            cost_bbox=args.set_cost_bbox,
                            cost_giou=args.set_cost_giou,
                            cost_alpha = args.cost_alpha,
                            cost_gamma = args.cost_gamma,
